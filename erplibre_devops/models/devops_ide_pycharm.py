...
class DevopsIdePycharm(models.Model):
    ...
    def action_cg_setup_pycharm_debug(self, ctx=None, log=None):
        for rec in self:
            if not log:
                log = rec.devops_workspace.devops_cg_erplibre_devops_log
            ...
            search_path = (
                "File"
                f' "{os.path.normpath(os.path.join(rec.devops_workspace.folder, "./addons"))}'
            )
    ...
    def add_breakpoint(self, file_path, line):
        url = file_path.replace(
            self.devops_workspace.folder, "file://$PROJECT_DIR$/"
        )
        ...
        workspace_xml_path = os.path.join(
            self.devops_workspace.folder, ".idea", "workspace.xml"
        )
